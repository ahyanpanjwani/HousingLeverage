 ...
 * adjusting their q.
 *
 * In order to implement this with reasonable efficiency, I will nest the buy/sell problem within the refinance
 * problem. The refinance loops start with asset and CS, I will nest an additional loop for q and the value
 * will be the max of the value from refinancing and buying/selling. We'll see how this goes but run time should
 * increase somewhat.
 *
 * The next iteration of the model will allow households to default. Default and rent are not in this iteration
 * of the model, i.e. LifeCycleProblemD.java.
 *
 * Jun 25th, 2020
 */

public class LifeCycleProblemD {
    ...
    public static void main(String args[]){
        ...
        LifeCycleProblemD lifeCycleProblemD= new LifeCycleProblemD();
        double[] agrid = lifeCycleProblemD.agrid(na, amin, amax);                     // asset grid
        double[] egrid = lifeCycleProblemD.egrid(ne, sigma_eps, lambda_eps, m);       // productivity grid
        Matrix P = lifeCycleProblemD.P(ne, sigma_eps, lambda_eps, m, egrid);          // productivity transition matrix
        double[] qgrid = lifeCycleProblemD.qgrid(nq, qmin, qmax);                     // quantity grid
        double[] lgrid = lifeCycleProblemD.lgrid(CSfilePath, nCS);                    // oltv grid
        double[] mgrid = lifeCycleProblemD.mgrid(CSfilePath, nCS);                    // mortgage rate grid
        double[] ngrid = lifeCycleProblemD.ngrid(N + 1, 0, N);               // mortgage age grid
        double[][][][][][] V = lifeCycleProblemD.V(T, na, ne, nq, nCS, N + 1);     // value function for rest of life
        double[][] V_0 = lifeCycleProblemD.V_0(na, ne);                                // value function for newborns
        double[][] Q_0 = lifeCycleProblemD.V_0(na, ne);                                // policy function for quantity bought
        ...
        for (int age = T - 1; age >= 1; age--){
            ...
            IntStream.range(0, Z).parallel().forEach(z ->{
                ...
                V[finalAge][node.get(0)][node.get(1)][node.get(2)][node.get(3)][node.get(4)]
                        = lifeCycleProblemD.value(finalAge, node.get(0), node.get(1), node.get(2), node.get(3), node.get(4),
                        T, na, ne, nq, nCS, N,
        ...
        IntStream.range(0, na * ne).parallel().forEach(z -> {
            ...
            V_0[node.get(0)][node.get(1)] = lifeCycleProblemD.value_0(age, node.get(0), node.get(1), T, na, ne, nq, nCS, N,
                    agrid, egrid, P, qgrid, lgrid, mgrid, ngrid, V, alpha, beta, w, r, pH, muH, N)[0];
            Q_0[node.get(0)][node.get(1)] = lifeCycleProblemD.value_0(age, node.get(0), node.get(1), T, na, ne, nq, nCS, N,
