 ...
 * expectations is calculated in the Grids.java file and called into LifeCycleProblemF.java file. The expectations are
...
public class LifeCycleProblemF {
    ...
    public static void main(String[] args){
        ...
        IntStream.range(0, Z).parallel().forEach(z ->{
            ...
            capture[age_T][z] = lifeCycleProblemF.value_T(age_T, node.get(0), node.get(1), node.get(2), node.get(3), node.get(4),
                    T, na, agrid, egrid, P, qgrid, lgrid, mgrid, ngrid, alpha, beta, w, r, pH, muH, N);
        });
        ...
        for (int age = T - 2; age > 0; age--){
            ...
            IntStream.range(0, Z).parallel().forEach(z ->{
                ...
                capture[age_t][z] = lifeCycleProblemF.value_t(age_t, node.get(0), node.get(1), node.get(2), node.get(3), node.get(4),
                        T, na, nq, nCS,
                        agrid, egrid, qgrid, lgrid, mgrid, ngrid, finalExpected_t, alpha, beta, w, r, pH, muH, N);
            });
        }
