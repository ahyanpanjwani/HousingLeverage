...
public class Grids {
    ...
    double[] mgrid(String filePathForCS, int nm){
        ...
        try {
            ...
            while ((row = bufferedReader.readLine()) != null & it < nm) {
                String[] data = row.split(",");
                mgrid[it] = Double.parseDouble(data[2]);
                it++;
            }
        }catch (IOException e){e.printStackTrace();}
        return mgrid;
    ...
    double[][] creditSurface(String filePathForCS, int nf, int nCS){
    ...
    Matrix FicoFicoRates(int numberOfFicoBins, String ficoFicoRatesPath){
        List<String[]> rowList2 = new ArrayList<String[]>();
        try(BufferedReader br = new BufferedReader(new FileReader(ficoFicoRatesPath))){
            String line;
            while((line = br.readLine()) != null){
                String[] lineItems = line.split(",");
                rowList2.add(lineItems);
        ...
        String[][] FicoFico = new String[rowList2.size()][];
        for (int i = 0; i < rowList2.size(); i++){
            String[] row = rowList2.get(i);
            FicoFico[i] = row;
        }
    ...
    double[] Parameters(String inputParametersFilePath){
        List<String[]> rowList2 = new ArrayList<String[]>();
        try(BufferedReader br = new BufferedReader(new FileReader(inputParametersFilePath))){
            String line;
            while ((line = br.readLine()) != null){
                String[] lineItems = line.split(",");
                rowList2.add(lineItems);
        ...
        double[] parameters = new double[rowList2.size()];

        for (int i = 0; i < parameters.length; i++){
            parameters[i] = Double.parseDouble(rowList2.get(i)[1]);
        }
        return parameters;
    }
